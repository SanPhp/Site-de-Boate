...
{ ?>
	...
	<?php
		...
		<li><a href="<?php echo $link ?>" title="<?php echo $titulo ?>"><img src="<?php echo DECK34_UPLOADPATH_BANNER_FRONTEND . $banner ?>" alt="<?php echo $alt ?>" class="cube" width="1000" height="355"></a>
...
<!--------------------------- LISTAR CARTAZ PROX EVENTO BACKEND PARA DELETAR --------------------------------->
...
{ ?>
	...
	<?php
		...
		<figure>
			<img src="<?php echo DECK34_UPLOADPATH_BANNER . $banner ?>" alt="" width="30%" title="<?php echo $titulo ?>">
...
<!--------------------------- LISTAR CARTAZ PROX EVENTO BACKEND --------------------------------->
...
<!--------------------------- LISTAR CARTAZ PROX EVENTO FRONTEND --------------------------------->
...
{ ?>
	...
	<?php
		$dbc = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
		$query = "SELECT * FROM banner_prox_evento WHERE tipo = 'cartaz' LIMIT 1";
		$sql = mysqli_query($dbc, $query)
			or die("mysqli_error");
		...
		<img src="<?php echo DECK34_UPLOADPATH_BANNER_FRONTEND . $banner ?>" alt="<?php echo $alt ?>" width="480" height="610" title="<?php echo $titulo ?>">
...
<!--------------------------- LISTAR PROGRAMACAO DA INDEX FRONTEND --------------------------------->
...
<!--------------------------- ALTERAR PROGRAMACAO DA PAGINA EVENTOS --------------------------------->
...
{ ?>
	...
	<form action="" method="post" enctype="multipart/form-data">
		...
		<textarea name="programacao_pagina"><?php if (!empty($programacao_pagina)) echo $programacao_pagina ?></textarea>
		...
		if (isset($_POST['submit'])) {
			...
			if (!empty($titulo)) {
				...
				if (($capa_type == 'image/jpg') || ($capa_type == 'image/jpeg') || ($capa_type == 'image/pjpeg') || ($capa_type == 'image/png') && ($capa_size > 0) && ($capa_size <= DECK34_MAXFILESIZE_CAPA)) {
					...
					echo '<p class="error">Por favor insira arquivos do tipo JPG ou PNG!</p>';
				} // else type
...
<!--------------------------- LISTAR CATEGORIA GALERIA BACKEND -------------------------------->
...
{ ?>
	...
	<?php
		...
		<?php
				...
				?>
		<figure>
			<figcaption><a href="javascript:if(confirm('Tem Certeza que Deseja Excluir este Album?')) {location='index.php?capa_miniatura=<?php echo $capa_miniatura ?>&id_categoria_galeria=<?php echo $id_categoria_galeria ?>';}" title="Deletar Album?">Deletar album ( <?php echo $conta_linha ?> )</a></figcaption>
...
<!--------------------------- LISTAR CATEGORIA GALERIA FRONTEND -------------------------------->
...
<!--------------------------- DELETAR CATEGORIA GALERIA BACKEND -------------------------------->
...
<!--------------------------- INSERIR IMAGEM GALERIA BACKEND -------------------------------->
...
<!--------------------------- LISTAR IMAGENS GALERIA BACKEND -------------------------------->
...
<!--------------------------- DELETE IMAGENS GALERIA BACKEND -------------------------------->
...
<!--------------------------- LISTAR IMAGENS GALERIA FRONTEND INDEX -------------------------------->
...
<!--------------------------- LISTAR IMAGENS EVENTO GALERIA FRONTEND PAGINA FOTOS_EVENTO -------------------------------->
...
{ ?>
	...
	<?php
		...
		<div class="videos_block">
			...
			<a href="<?php echo $embed_video ?>" onclick="return hs.htmlExpand(this, {objectType: 'iframe', width: 600, height: 481, 
		   allowSizeReduction: false, wrapperClassName: 'draggable-header no-footer', 
		   preserveContent: false, objectLoadTime: 'after'})" class="highslide">

				<img src="<?php echo DECK34_UPLOADPATH_CAPA_VIDEO . $imagem_video ?>" alt="<?php echo $titulo_video ?>" width="400" height="225"></a><br>
			<a href="videos45.php?imagem_video=<?php echo $imagem_video ?>&amp;id_video=<?php echo $id_video ?>" title="Deletar Video?">Deletar Video</a>
		</div>
		<!--videos_block-->
...
<!--------------------------- LISTAR VIDEO PAGINA INDEX FRONTEND -------------------------------->
